...
test('obsolete gift rewards cannot enter an interaction allocation',()=>{expect(()=>allocation(0,{},'promotional_gift')).toThrow('Invalid transaction type');expect(allocation(5,{},'paid_gift').transactionType).toBe('paid_gift');});
...
test('seven-day schedule has deterministic development rewards and preserves all balances', () => {
  ...
  expect(total).toEqual({ freeMessages: 11, freeVideoSeconds: 35, quickMatchCount: 1 });
});
