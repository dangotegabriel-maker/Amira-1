...
const {nextCheckInDay}=require('./checkInDomain');

const createConsumerRewards = ({ db, FieldValue, HttpsError, clock = () => Date.now() }) => {
  ...
  const getState = (uid, claim = false) => db.runTransaction(async (tx) => {
    ...
    const now = clock(), dateKey = utcDateKey(new Date(now));
    ...
    const day = nextCheckInDay(previous.checkIn,dateKey);
    const response = { balances: E.normalizeRewards(previous), checkIn: previous.checkIn || { totalClaims: 0, lastClaimDate: null },
      ...
      earnedVideoEnabled: policy.enableEarnedVideoSeconds, reward: history.exists ? E.normalizeRewards(history.data()?.reward) : null };
