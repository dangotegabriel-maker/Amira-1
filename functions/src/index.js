...
const consumerRewardTasks=require('./consumerRewardTasks').createConsumerRewardTasks({db,FieldValue,HttpsError});
...
const sponsoredInviteService=require('./sponsoredInviteService').createSponsoredInviteService({db,FieldValue,HttpsError,
  createRtc:(callId,consumerUid,hostUid)=>{let consumerRtcUid=randomRtcUid(),hostRtcUid=randomRtcUid();while(hostRtcUid===consumerRtcUid)hostRtcUid=randomRtcUid();return {channelName:`amira_${callId}_${crypto.randomBytes(8).toString('hex')}`,uids:{[consumerUid]:consumerRtcUid,[hostUid]:hostRtcUid}};}});
...
const callResult=(id,data)=>({callId:id,status:data.status,callerId:data.callerId,receiverId:data.receiverId,participantIds:data.participantIds,ratePerMinute:data.ratePerMinute,economicsSnapshot:data.economicsSnapshot||null,previewEligible:data.previewEligible,billingMode:data.billingMode,expiresAtMs:data.expiresAtMs});

exports.startVideoCall=onCall(callable,async(request)=>{try{const callerId=requireAuth(request),creatorId=textId(request.data?.creatorId,'creatorId');if(request.data?.termsVersion!=='automatic-paid-v3'||Object.keys(request.data||{}).some(k=>!['creatorId','termsVersion'].includes(k)))throw new HttpsError('invalid-argument','Current automatic paid call terms must be accepted.');await recovery.recoverLock(callerId);await recovery.recoverLock(creatorId);const callRef=db.collection('calls').doc(),callerRef=db.doc(`users/${callerId}`),creatorRef=db.doc(`users/${creatorId}`),callerLock=db.doc(`activeCallLocks/${callerId}`),creatorLock=db.doc(`activeCallLocks/${creatorId}`),entitlementRef=db.doc(`users/${callerId}/entitlements/dailyPreview`);return await db.runTransaction(async(tx)=>{const [callerSnap,creatorSnap,callerLockSnap,creatorLockSnap,entSnap,rewardsSnap,configSnap,vipMembershipSnap,vipConfigSnap,...blocks]=await Promise.all([tx.get(callerRef),tx.get(creatorRef),tx.get(callerLock),tx.get(creatorLock),tx.get(entitlementRef),tx.get(db.doc('consumerRewards/'+callerId)),tx.get(db.doc('economyConfig/current')),tx.get(db.doc('vipMemberships/'+callerId)),tx.get(db.doc('vipConfig/current')),...blockedRefs(callerId,creatorId).map((ref)=>tx.get(ref))]);const creator=creatorSnap.data();D.validateStart({authUid:callerId,creatorId,creator,blocked:blocks.some((s)=>s.exists),callerLock:callerLockSnap.exists,creatorLock:creatorLockSnap.exists});if(!callerSnap.exists)throw new Error('caller-missing');if(!isConsumer(callerSnap.data()))throw new HttpsError('permission-denied','Only consumers can start paid host calls.');const rate=creator.hostProfile?.videoRateCredits;if(!Number.isInteger(rate)||rate<=0)throw new Error('invalid-rate');const now=Date.now(),dateKey=D.utcDateKey(),eligible=D.previewEligible(entSnap.data(),dateKey),channelName=`amira_${callRef.id}_${crypto.randomBytes(8).toString('hex')}`;const pricing=require('./vipDomain').callPricing({hostRateCredits:rate,entitlement:vipMembershipSnap.data(),user:callerSnap.data(),nowMs:now,config:vipConfigSnap.data()});const economicsSnapshot={version:'call-economics-v3',baseRatePerMinute:rate,consumerRatePerMinute:pricing.consumerRateCredits,hostEarningBasisPerMinute:pricing.hostEarningBasisCredits,platformAbsorptionPerMinute:pricing.platformAbsorptionCredits,vipPolicyVersion:pricing.policyVersion,billingIncrementSeconds:D.BILLING_INCREMENT_SECONDS,rateUnit:'credits_per_minute',automaticPaidContinuation:true,disclosureAccepted:true,disclosureSource:'consumer_initiated_direct_call',disclosedAtMs:now};const entry=require('./callPreflight').callPreflight({caller:callerSnap.data(),rate:pricing.consumerRateCredits,dailyEligible:eligible,rewards:rewardsSnap.data(),config:configSnap.data()});economicsSnapshot.freeVideoSource=entry.freeVideoSource;economicsSnapshot.freeVideoSeconds=entry.freeVideoAllowanceSeconds;if(!entry.allowed)throw new HttpsError('resource-exhausted','You need Credits to start this video call.',{reason:'insufficient_call_credits',ratePerMinute:pricing.consumerRateCredits,minimumCredits:entry.minimumCredits});let callerRtcUid=randomRtcUid(),creatorRtcUid=randomRtcUid();while(creatorRtcUid===callerRtcUid)creatorRtcUid=randomRtcUid();const data={accountingVersion:3,economicsSnapshot,id:callRef.id,callerId,receiverId:creatorId,participantIds:[callerId,creatorId],status:'ringing',createdAt:FieldValue.serverTimestamp(),createdAtMs:now,expiresAt:Timestamp.fromMillis(now+D.RING_TIMEOUT_SECONDS*1000),expiresAtMs:now+D.RING_TIMEOUT_SECONDS*1000,acceptedAt:null,connectedAt:null,endedAt:null,durationSeconds:0,paidDurationSeconds:0,ratePerMinute:rate,previewEligible:eligible,previewConsumed:false,billingMode:eligible?D.BILLING_MODES.PREVIEW:D.BILLING_MODES.AWAITING_PAID_CONFIRMATION,billedCredits:0,settledIncrements:0,paidStartedAt:null,paidStartedAtMs:null,endedBy:null,endReason:null,rtc:{channelName,uids:{[callerId]:callerRtcUid,[creatorId]:creatorRtcUid}},rtcAcks:{},lastRtcPresenceAt:null};tx.create(callRef,data);const lock={callId:callRef.id,createdAt:FieldValue.serverTimestamp(),expiresAtMs:data.expiresAtMs};tx.create(callerLock,lock);tx.create(creatorLock,lock);return callResult(callRef.id,data);});}catch(e){throw mapError(e);}});

exports.respondToVideoCall=onCall(callable,async(request)=>{try{const uid=requireAuth(request),callId=textId(request.data?.callId,'callId'),action=request.data?.action;if(!['accept','decline'].includes(action))throw new HttpsError('invalid-argument','Action must be accept or decline.');const callRef=db.doc(`calls/${callId}`);return await db.runTransaction(async(tx)=>{const callSnap=await tx.get(callRef);if(!callSnap.exists)throw new HttpsError('not-found','Call not found.');const call=callSnap.data(),creatorRef=db.doc(`users/${call.receiverId}`),callerRef=db.doc(`users/${call.callerId}`);const [creatorSnap,callerSnap,...blocks]=await Promise.all([tx.get(creatorRef),tx.get(callerRef),...blockedRefs(call.callerId,call.receiverId).map(ref=>tx.get(ref))]);D.validateAcceptance({authUid:uid,call,creator:creatorSnap.data()});const lockRefs=call.participantIds.map(id=>db.doc(`activeCallLocks/${id}`)),lockSnaps=await Promise.all(lockRefs.map(ref=>tx.get(ref)));if(action==='decline'){tx.update(callRef,{status:'rejected',endedAt:FieldValue.serverTimestamp(),endedBy:uid,endReason:'declined',billingMode:D.BILLING_MODES.ENDED});tx.set(db.doc(`callHistory/${callId}`),{callId,participantIds:call.participantIds,callerId:call.callerId,receiverId:call.receiverId,createdAt:call.createdAt,endedAt:FieldValue.serverTimestamp(),durationSeconds:0,paidDurationSeconds:0,ratePerMinute:call.ratePerMinute,billedCredits:0,billingMode:call.billingMode,previewConsumed:false,endReason:'declined',status:'rejected'});lockSnaps.forEach((lock,i)=>{if(lock.data()?.callId===callId)tx.delete(lockRefs[i]);});return {callId,status:'rejected'};}if(!callerSnap.exists||!isConsumer(callerSnap.data())||callerSnap.data().isDemo||blocks.some(s=>s.exists))throw new HttpsError('permission-denied','This call can no longer be answered.');if(lockSnaps.some(lock=>!lock.exists||lock.data()?.callId!==callId))throw new HttpsError('already-exists','A participant is already in another call.');tx.update(db.doc(`activeCallLocks/${call.callerId}`),{expiresAtMs:Date.now()+require('./connectionAccounting').CONNECTING_TIMEOUT_MS});tx.update(db.doc(`activeCallLocks/${call.receiverId}`),{expiresAtMs:Date.now()+require('./connectionAccounting').CONNECTING_TIMEOUT_MS});tx.update(callRef,{status:'connecting',acceptedAt:FieldValue.serverTimestamp(),connectingDeadlineMs:Date.now()+require('./connectionAccounting').CONNECTING_TIMEOUT_MS,expiresAtMs:Date.now()+require('./connectionAccounting').CONNECTING_TIMEOUT_MS});tx.update(creatorRef,{'hostStatus.availability':'busy','hostStatus.preCallAvailability':creatorSnap.data().hostStatus?.availability||'online'});return {callId,status:'connecting'};});}catch(e){throw mapError(e);}});
...
exports.reconcileExpiredVideoCalls=onSchedule({region,schedule:'every 1 minutes'},async()=>{
  ...
  for(const entry of snapshot.docs){
    if([2,3].includes(entry.data().accountingVersion))await recovery.run(entry.id,null,'reconcile');
...
exports.getConsumerRewardTasks=onCall(callable,async(request)=>{
  try { return await consumerRewardTasks.dashboard(requireAuth(request)); } catch(e) { throw mapError(e); }
});
exports.claimConsumerRewardTask=onCall(callable,async(request)=>{
  try { return await consumerRewardTasks.claim(requireAuth(request),request.data||{}); } catch(e) { throw mapError(e); }
});
...
exports.sendSponsoredCallInvite=onCall(callable,request=>sponsoredInviteService.send(requireAuth(request),request.data||{}));
exports.getSponsoredCallInvites=onCall(callable,request=>sponsoredInviteService.pending(requireAuth(request),request.data||{}));
exports.respondToSponsoredCallInvite=onCall(callable,request=>sponsoredInviteService.respond(requireAuth(request),request.data||{}));
exports.reconcileSponsoredCallInvites=onSchedule({region,schedule:'every 5 minutes'},()=>sponsoredInviteService.reconcile());
