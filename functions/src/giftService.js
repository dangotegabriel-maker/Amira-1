...
const createGiftService=({db,FieldValue,HttpsError,now=()=>Date.now()})=>{
 ...
 const send=async(uid,input={})=>{
  if(!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).some(k=>!['hostUid','giftId','source','requestId','callId'].includes(k)))fail('invalid-argument','Invalid Gift request.');
  text(uid,'Consumer UID');const hostUid=text(input.hostUid,'Host UID'),giftId=text(input.giftId,'Gift ID'),requestId=text(input.requestId,'Request ID'),source=guard(()=>G.source(input.source),'invalid-argument'),callId=input.callId===undefined?null:text(input.callId,'Call ID');
  if((source==='video_call')!==Boolean(callId))fail('invalid-argument','Video Call Gifts require valid call context.');
  ...
  return db.runTransaction(async tx=>{
   ...
   const [consumerSnap,hostSnap,left,right,configSnap,walletSnap,existing,hostMoney,hostRecord,platform,aggregate,conversation,message,callSnap]=await Promise.all(Object.values(refs).map(ref=>tx.get(ref)).concat([tx.get(conversationRef),tx.get(messageRef),callId?tx.get(db.doc(`calls/${callId}`)):Promise.resolve(null)]));
   if(existing.exists){const prior=existing.data();if(prior.consumerUid!==uid||prior.requestId!==requestId||prior.hostUid!==hostUid||prior.giftId!==giftId||prior.source!==source||(prior.callId||null)!==callId)fail('already-exists','Gift request identity conflict.');return {transactionId,giftId:prior.giftId,chargedCredits:prior.priceCredits,idempotent:true};}
   ...
   if(source==='video_call'){const call=callSnap?.data(),participants=call?.participantIds;if(!callSnap?.exists||call.status!=='connected'||call.callerId!==uid||call.receiverId!==hostUid||!Array.isArray(participants)||participants.length!==2||!participants.includes(uid)||!participants.includes(hostUid))fail('failed-precondition','This Video Call is not active.',{reason:'inactive_video_call'});}
   ...
   const record={transactionId,requestId,consumerUid:uid,hostUid,giftId,giftSnapshot:snapshot,priceCredits:gift.priceCredits,source,...(callId?{callId}:{}),catalogVersion:cfg.version,economicsVersion:cfg.economics.version,hostCreditsEquivalent:split.hostCreditsEquivalent,platformCreditsEquivalent:split.platformCreditsEquivalent,creditLedgerEntryId:ledgerRef.id,hostEarningRecordId:refs.hostRecord.id,status:'succeeded',createdAt:timestamp,createdAtMs:openedAt.getTime(),version:1};
