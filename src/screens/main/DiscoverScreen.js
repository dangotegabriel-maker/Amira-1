...
const DiscoverScreen = () => {
  ...
  const discoverUsers = [
    {
      ...
      isOnline: true, giftsReceived: 450, responseRate: 98
    },
    {
      ...
      isOnline: true, giftsReceived: 210, responseRate: 92
    }
  ];
  ...
  const targetDisplayUser = discoverUsers[0];
  ...
  }, [targetDisplayUser, currentPhotoIndex]);
  ...
  const handleLike = () => { animateButton(likeScale); hapticService.mediumImpact(); };
  const handleDislike = () => { animateButton(dislikeScale); hapticService.lightImpact(); };
  const handleSuperlike = () => { animateButton(superlikeScale); hapticService.heavyImpact(); };
...
const styles = StyleSheet.create({
  ...
  storiesContainer: { width: '100%', backgroundColor: COLORS.white, paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: '#F0F0F0', paddingTop: 50 },
