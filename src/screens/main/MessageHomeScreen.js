...
const setCalls = items=>setCallRecord({ownerUid:user.uid,items});
...
useEffect(()=>{let alive=true;setCalls([]);setCallsLoading(true);setCallsError('');
callHistoryService.list().then(async(records)=>{
const ids=[...new Set(records.flatMap(record=>record.participantIds||[]).filter(id=>id!==user.uid))];
const states=await Promise.all(ids.map(async uid=>[uid,await presenceService.get(uid).catch(()=>null)]));
const online=Object.fromEntries(states.map(([uid,state])=>[uid,presenceService.isOnline(state)]));
const grouped=callHistoryService.groupContacts(records,user.uid,online);
const identities=await publicIdentityService.calls(grouped.map(item=>item.lastCall.callId || item.lastCall.id));
const byCall=new Map(identities.map(item=>[item.callId,item]));
const profiles=grouped.map(item=>{const result=byCall.get(item.lastCall.callId || item.lastCall.id);return {...item,profile:result?.identity,canInteract:result?.canInteract===true};});
if(alive)setCalls(profiles);
}).catch(()=>{if(alive)setCallsError('Unable to load call identities. Please retry.');}).finally(()=>{if(alive)setCallsLoading(false);});
