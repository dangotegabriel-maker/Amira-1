...
export const getCreatorCardState = (user, applicationStatus = '') => {
  if (isApprovedHost(user)) return CREATOR_CARD_STATES.APPROVED;
  ...
  if (['draft','in_progress','incomplete'].includes(status)) return CREATOR_CARD_STATES.IN_PROGRESS;
...
export const CREATOR_CARD_COPY = Object.freeze({
  ...
  pending: { title: 'Application Under Review', description: 'Your application is waiting for review.', cta: 'Pending Review' },
