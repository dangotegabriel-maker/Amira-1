...
async function main(){
 ...
 const all=(await activity.list('h',{tab:'All'})).events;check(new Set(types(all)).size,4);check(types(all).includes('Gifts'),false);check(all.map(event=>event.id),[...all].sort((a,b)=>b.timestampMs-a.timestampMs||a.id.localeCompare(b.id)).map(event=>event.id));
 ...
 check((await activity.list('h',{tab:'Gifts'})).events,[]);
 ...
 await db.doc('consumerRewards/c').set({freeMessages:0});await social.sendText('h',{receiverId:'c',messageId:'host-message',text:'Hello'});check((await db.doc('consumerRewards/c').get()).data().freeMessages,0);await assert.rejects(social.sendText('c',{receiverId:'h',messageId:'no-pass',text:'Reply'}),error=>error.code==='resource-exhausted');checks++;
