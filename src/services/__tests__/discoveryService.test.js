...
test('Host Consumer discovery uses own authenticated compact backend feed in both directions',async()=>{invokeSocial.mockResolvedValue({people:[]});expect(await discoveryService.getConsumersForHosts('ignored-owner')).toEqual([]);expect(await discoveryService.getFollowingConsumers('ignored-owner')).toEqual([]);expect(invokeSocial.mock.calls).toEqual([['getHostConnectConsumers',{tab:'For You'}],['getHostConnectConsumers',{tab:'Following'}]]);});
