...
async function main(){
 ...
 const fullConsumer=await activity.consumerProfile('h','c');safe(fullConsumer);check(Object.keys(fullConsumer).sort(),['age','amiraId','countryCode','gender','hostProfile','hostStatus','profilePic','role','uid','username','vipActive']);check(fullConsumer.vipActive,false);check(fullConsumer.amiraId,(await db.doc('users/c').get()).data().amiraId);check(Object.hasOwn(fullConsumer,'level'),false);
